...
| Google OAuth restricted-scope verification path | the top business risk — see [credentials](../security/credentials.md). **Passed:** answered, and the assessment deferred to general availability |
...
| Toolkit residue, and a warm body view alongside it | Figures exist for both architectures and both platforms, separately, and NFR-8 and NFR-9 have been re-derived from them together with the reading peak — see D-105 below, which is what "passes" means for a spike whose output is a number. **Not passed:** the instrument exists (the automated protocol, run natively on both macOS architectures by the per-integration tier), and one cell has provisional figures from the interim rig — macOS on Apple silicon, recorded in [memory pressure](../runtime/memory-pressure.md#provisional-figures). The Intel and Linux cells and the reference rig do not exist yet, so nothing has been re-derived |
...
| Google OAuth restricted-scope verification path | The requirements, the cost and the timeline are documented well enough for [R-1](../open-questions.md) to become a budget decision rather than an unknown. It does not pass by being granted; it passes by being answerable. **Passed:** [credentials](../security/credentials.md) records the requirements, both cost branches and a one-to-three-month timeline, and the decision taken on them |
...
credentials only in the OS store, NFR-24 never a listening socket. NFR-14 and NFR-52 are the
no-unbounded-cache rule, which between them cover the whole of disk; NFR-16 and NFR-19 are
crash-consistency and surviving hostile MIME, and NFR-51 says "from the first commit" in its own text.

NFR-54 and NFR-55 are here for the same reason as each other: neither fails loudly. A subject that renders
backwards looks like a subject, and a log line written in the wrong shape cannot be unwritten on a machine
that already ran that build. There is no phase at which shipping without either would be noticed, which is
what makes deferring them different from deferring a feature.
