...
| **Shell** | The native application interface. AppKit on macOS, GTK4 on Linux. Not a web engine, and nothing in it is authoritative. It has two lifetimes — see the two rows below — and an unqualified "shell" means the window shell |
| **Application shell** | The part of the shell resident for the life of the process: the tray or menu-bar item, the application menu, notification delivery, and the host callbacks of [D-67](architecture/view-protocol.md). Holds no view hierarchy. See [UI shell](architecture/ui-shell.md) |
| **Window shell** | The part created with a window and destroyed with it: list, sidebar, reader chrome. What every statement about the shell being disposable refers to, including L3 in [memory pressure](runtime/memory-pressure.md) |
...
| **Policy tier** | One of the six network behaviour levels: Unrestricted, Conservative, Minimal, Offline-portal, Offline-no-path, and Paused. The first five are derived from the network; Paused is entered by the user under FR-22 or by FR-36's cap reaching it, and is a tier rather than a separate mechanism under [D-58](runtime/network-conditions.md). See [network conditions](runtime/network-conditions.md) |
...
| **Fidelity corpus** | The real-world message set that rendering correctness is gated on. One of three: the **scale corpus** measures performance and resource targets, this one measures whether a message renders, and the **relevance corpus** measures whether search returned the right message first. See [reference environment](product/reference-environment.md) |
