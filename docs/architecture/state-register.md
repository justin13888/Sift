...
| Resource unavailable | the count, and per resource why — decrypt failure, missing blob, timeout, revoked document, or a caught panic | [D-91](resource-broker.md), which distinguishes this from both a block and a fabricated address; [D-47](overview.md) for the caught panic |
